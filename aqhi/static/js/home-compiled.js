...
function initCityWeather(cityEngName, cityPanel) {
  ...
  $.getJSON(`static/weather/test-${ cityEngName }.json`).done(function (jsonData) {
    ...
    curWeatherIcon.removeClass().addClass(`wi ${ weatherIconClassNames[nowWeatherData.cond.code][isDay ? 'day' : 'night'] }`);
    ...
    nextDayWeatherFirstRow.find('.next-day-weather-header').text(isDay ? '今天白天' : '今天夜间');
    nextDayWeatherFirstRow.find('.next-day-weather-text').text(isDay ? todayWeatherData.cond.txt_d : todayWeatherData.cond.txt_n);
    nextDayWeatherFirstRow.find('i').removeClass().addClass(`wi ${ isDay ? weatherIconClassNames[todayWeatherData.cond.code_d].day : weatherIconClassNames[todayWeatherData.cond.code_n].night }`);
    nextDayWeatherSecondRow.find('.next-day-weather-header').text(isDay ? '今天夜间' : '明天白天');
    nextDayWeatherSecondRow.find('.next-day-weather-text').text(isDay ? todayWeatherData.cond.txt_n : tomorrowWeatherData.cond.txt_d);
    nextDayWeatherSecondRow.find('i').removeClass().addClass(`wi ${ isDay ? weatherIconClassNames[todayWeatherData.cond.code_n].night : weatherIconClassNames[tomorrowWeatherData.cond.code_d].day }`);

    var hourlyForecasts = weatherData.hourly_forecast;
    var hourlyForecastList = cityPanel.find('.hourly-weather-list');
    // Add now temp first
    hourlyForecastList.empty().append(getNewHourlyWeatherListItem('<strong>现在</strong>', nowWeatherData.tmp));
    ...
    dailyForecastData.forEach(function (forecast) {
      ...
      dailyForecastList.append(getNewDailyWeatherItem(`周${ zhWeekDays[curWeekday] }`, forecast.cond.txt_d, weatherIconClassNames[forecast.cond.code_d].day, forecast.tmp.max, forecast.tmp.min));
    });
  }).done(function () {
    // Hide loading effect
